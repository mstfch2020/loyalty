...
import {
  ...
  CustomerScenario,
  CustomerSubGrid,
} from 'src/app/@core/data/loyalty/customer.model';
...
export class CustomerEditComponent extends BaseSearch implements OnInit {
  handleDiscountShowHistory(customerInfo: CustomerInfo) {
    this.customerInfoList.forEach((a) => {
      a.showCommissionHistory = false;
      a.showDiscountHistory = false;
    });
    customerInfo.showDiscountHistory = !customerInfo.showDiscountHistory;
  }

  handleCommissionShowHistory(customerInfo: CustomerInfo) {
    this.customerInfoList.forEach((a) => {
      a.showCommissionHistory = false;
      a.showDiscountHistory = false;
    });
    customerInfo.showCommissionHistory = !customerInfo.showCommissionHistory;
  }
  ...
  override search(request: any) {
    if (Utility.isNullOrEmpty(this.id)) {
      return;
    }
    request.id = this.id;
  ...
  override ngOnInit(): void {
    ...
    this.service.customerSubGrid$.subscribe(
      (value) => (this.subCustomerDetailList = value)
    );
    this.service.scenarioCustomer$.subscribe(
      (value) => (this.customerScenarioList = value)
    );
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.id) {
        ...
        this.service
          .GetPromoterDiscountCodesGeneralInfo(this.id)
          .subscribe((result) => {
  ...
  createNewDiscountCode() {
    ...
    const modalRef = this.modalService.open(CreateDiscountCodeDialogComponent, {
      size: 'lg',
      centered: true,
    });
